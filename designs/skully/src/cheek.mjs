export const cheek = {
  ...
  options: {
    size: {
      ...
      toAbs: (value, { measurements }, mergedOptions) =>
        value * ((measurements?.head ? measurements.head : 596) / 929.5) * 2 * 175,
      menu: 'fit',
    },
  },
  ...
  draft: ({
    ...
    const sizeFactor =
      (('head' in measurements ? measurements.head : 596) / 929.5) * options.size * 2
    ...
    paths.nose = new Path()
      ...
      .addText(complete ? 'skully:nose' : '', textAttribute)
      .addText(complete ? ' (10)' : '', textAttribute)
      .addClass('hidden')
    ...
    points.gridAnchor = points.point4.clone()
    ...
    macro('hd', {
      ...
      to: points.point7,
      y: points.point6.y - sa - 25,
      id: 'bottomNoseToRigthEye',
    })
    macro('hd', {
      from: points.point1,
      to: points.point7,
      y: points.point6.y - sa - 15,
      id: 'topNoseToRigthEye',
    })
    macro('hd', {
      from: points.point0,
      to: points.point7,
      y: points.point6.y - sa - 5,
      id: 'foreheadToRightEye',
    })
    macro('hd', {
      from: points.point9,
      to: points.point7,
